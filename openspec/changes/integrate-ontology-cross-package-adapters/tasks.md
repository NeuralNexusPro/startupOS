...
- [ ] 3.1 `ONT8-T1-E`（可与 3.2 并行；依赖：2.2；角色：Web adapter subagent；写入：`packages/web/src/app/api/` ONT8 route 与 route tests）实现参数/身份解析、Core 调用和 HTTP 映射；以 Web contract tests 验证业务逻辑未进入 route、错误不泄露正文或路径。
