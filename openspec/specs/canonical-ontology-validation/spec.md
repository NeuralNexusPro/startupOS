# canonical-ontology-validation Specification

## Purpose
为 canonical ontology 和 Action 请求提供唯一、无副作用的结构化验证边界，使下游在读取事实或执行副作用前统一拒绝损坏引用、过期版本、非法状态和权限不足。
...
Validator SHALL 校验 Domain/Concept/Instance、Property、Relation、BusinessState、Transition、FactType、Rule、Action、DomainEvent 和 Projection 的所有显式引用存在且属于正确概念；损坏引用 MUST 返回结构化错误而非抛出或静默忽略。
