...
### IM原始消息与发送者边界（SENSE12-T8）

- 感知事件负责规则匹配、授权和目标路由；企微、飞书、钉钉的消息正文不得改写为事件分析或升级评估任务。
- Channel在模型边界编码原文与独立sender/conversation/origin/attachmentRefs字段；发送者显示名仅使用平台提供值，不猜测身份。
- 用户会话保存原文，发送者/会话字段写入metadata.channel；调用级元数据不得使用全局当前发言人变量。聊天与等待用户回复的长任务分支均传递完整消息。
- 不改写角色/Skill指令、不清理历史记忆、不新增模型调用；消息仍以user角色处理，不能绕过授权。非IM调用保持原行为。
