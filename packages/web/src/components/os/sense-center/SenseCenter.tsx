...
export function SenseCenter({ initialJevProvider }: { initialJevProvider?: JevProviderSummary } = {}): JSX.Element {
  ...
  useEffect(() => {
    if (!showConnectorForm && !showGrantForm && !showRuleWizard) return;
    const form = document.querySelector<HTMLElement>('main[aria-label="感知中心"] form[aria-label]');
    form?.scrollIntoView?.({ behavior: 'smooth', block: 'start' });
    form?.querySelector<HTMLElement>('input:not([disabled]), select:not([disabled]), textarea:not([disabled])')?.focus({ preventScroll: true });
  }, [editingConnector?.id, editingRule?.id, showConnectorForm, showGrantForm, showRuleWizard]);
  useEffect(() => {
...
function ruleSourceLabel(rule: PerceptionTriggerRule): string {
  const connectorId = rule.conditions.find((condition) => condition.path === 'connectorId' && condition.operator === 'equals')?.value;
