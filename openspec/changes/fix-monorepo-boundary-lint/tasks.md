...
- [x] 2.1 AG5-T1-I：批准后从 Proposal 分支建立独立 proposal-task/fix-monorepo-boundary-lint-1-checks 分支/worktree，委派一个 subagent 修正配置、解析和扫描入口；依赖：1.2；串行（配置与自测共用规则源）；负责 subagent：架构工具工程师；写入范围：.eslintrc.json/.eslintrc.cjs、package.json、scripts/check-architecture-boundaries.cjs；必需检查：self-test、根目录与 Web 目录一致性、pnpm lint、pnpm lint:boundaries；证据：commit、检查输出和真实存量违规列表，不把存量失败标为通过。
