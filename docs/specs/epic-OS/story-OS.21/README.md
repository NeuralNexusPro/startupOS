...
**状态：** Implemented（Windows 睡眠恢复待人工验收）
**优先级：** High  
...
- [x] Core 提供与业务无关的统一调度运行时，Desktop 不再为邮箱轮询维护独立 `setInterval`。
- [x] 调度项明确区分用户任务与隐藏的系统任务；系统任务不出现在普通定时任务列表中。
- [x] 邮箱连接器启停、配置变化和应用退出会正确注册、重排或注销其系统任务。
- [x] 同一调度点最多执行一次；不同邮箱彼此隔离，失败采用有界退避。
- [x] 用户任务现有持久化、运行记录和权限边界保持兼容。
- [x] 可观测数据能区分调度、业务执行与重试，不记录邮箱凭据或正文。
...
| 2026-09-18 | 1.1 | 完成统一 Runtime、Plugin Host 迁移、邮箱失败退避与自动化回归 | System Runtime |
