...
- [x] 3.3 执行 OpenSpec strict validation，合并 Proposal branch 到 `dev`，确认只保留用户既有未提交修改并清理 Task/Proposal worktree 与临时分支；依赖：3.2；写入范围：git 集成状态；角色：Proposal owner；验证：`git status`、`git worktree list` 和合并提交；证据：合并提交 `63cdd46`，Task/Proposal worktree 与分支已清理，既有 `agent-worker.mts` 修改仍保留。
