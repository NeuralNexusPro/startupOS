...
## 2026-09-13 — fix: AG2-T3 / fix-agent-model-error-reporting
...
## 2026-09-13 — fix: 9.41-T2 / fix-unfinished-task-reopen
...
## 2026-09-13 — fix：企微流式积压与确认后状态提交

**类型**：fix
**影响模块**：Core ChannelOutputDispatcher、企微插件
**摘要**：共享派发器用有界预取合并连续文本积压，保留原包回执与控制顺序；企微确认后更新文本，避免重试重复。136项联合回归及实际桌面包通过，模拟ACK对照98次请求/2633ms降到5次/167ms，正文与回执完整。测试包同时包含仅事件页刷新的感知配置修复，真实网络待人工复核。

## 2026-09-13 — release：OriginOS CE 0.2.2

**类型**：release
**影响模块**：桌面发布
**摘要**：将补丁版本从0.2.1升级到0.2.2，包含本轮会话恢复、感知配置、企微流式、IM文件回复及架构治理；通过desktop-v0.2.2标签触发正式发布。
