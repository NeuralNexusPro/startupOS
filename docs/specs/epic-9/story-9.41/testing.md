...
## 2026-09-22 OpenSpec 5.2 Verification Evidence

Story 9.41 的产品运行链路已实现并通过自动化回归：`pi-tasks` ledger 与 Evidence Gate、adapter/host invoke、Core coordinator 与 completion policy、Desktop IPC、Web 入口/卡片/控制均通过；`pnpm lint`、架构边界、自测、Core/Web/Desktop 构建和 OpenSpec strict validation 均通过。完整矩阵见
`openspec/changes/archive/2026-09-22-implement-agent-session-task-runtime/verification-evidence.md`。

仍待平台证据：Windows x64、macOS x64 和 macOS arm64 真包 smoke。`release/` 目前只有旧 0.2.4 artifacts，不能作为 0.3.1 验收依据。
