...
AC1：三个IM正文原样透传，不混入事件/规则/HITL分析说明。AC2：发送者ID/可用显示名、来源、会话类型及附件同时到达目标和模型，并独立持久化，不把群成员当所有者。AC3：原会话恢复与无回复句柄时仍传原文，权限/投递/日志保留。AC4：非IM兼容，无额外模型调用或历史清理。来源为2026-09-14用户明确的原始信息与发送者透传要求；测试见IR01–IR07。
