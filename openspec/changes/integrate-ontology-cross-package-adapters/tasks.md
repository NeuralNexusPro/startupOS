...
- [x] 3.2 `ONT8-T1-F`（可与 3.1 并行；依赖：2.2；角色：Desktop adapter subagent；写入：`packages/desktop/src/main/`、preload/IPC contract 与 tests）实现 sender/channel 校验、Core 调用和 `IpcResponse` 映射；以 IPC allowlist、无效 sender、版本冲突和打包模块解析测试验证边界。完成证据：新增 `ontology:cross-package:invoke` 精确 channel、main 控制器与 preload 精确方法；main 只信任当前 BrowserWindow sender 并注入 `desktop-sender:<senderId>`，不信任 renderer actor；9/9 定向 tests 覆盖 allowlist、无效 sender、v1/revision 校验、Core 成功/冲突映射、异常不泄露和缺失生产端口 `CAPABILITY_NOT_READY` fail closed；Desktop build 通过，`pnpm lint`、`pnpm lint:boundaries`、architecture self-test 与 strict validation 通过。
- [x] 3.3 `ONT8-T1-G`（串行；依赖：3.1、3.2；角色：Contract QA subagent；写入：跨包 parity fixture/tests）用同一输入矩阵比较 Web 与 Desktop 规范化响应；以成功、授权失败、版本冲突、幂等重试和 unavailable 场景证明 transport 语义等价。完成证据：新增跨 transport parity tests，同一输入矩阵驱动 Web route 与 Desktop IPC controller；5/5 tests 覆盖成功 data/revision、authorization、version conflict、重复请求幂等响应和缺失生产端口 `CAPABILITY_NOT_READY` unavailable，并验证 Web/Desktop actor 均由 transport 注入且 renderer 请求 actor 被覆盖；Web type-check 与 Desktop 9/9 tests/build 通过，`pnpm lint`、`pnpm lint:boundaries`、architecture self-test 与 strict validation 通过。
