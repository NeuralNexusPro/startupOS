...
import type { WeComBotClient, WeComBotClientFactory, WeComFrame, WeComSettings } from './types';

function defaultClientFactory(options: { botId: string; secret: string; wsUrl?: string }): WeComBotClient {
  return new AiBot.WSClient(options) as WeComBotClient;
}

function parseSettings(settings: Readonly<Record<string, unknown>>): WeComSettings {
  const botId = typeof settings.botId === 'string' ? settings.botId.trim() : '';
  if (!botId) throw new Error('WECOM_BOT_ID_REQUIRED');
  const websocketUrl = typeof settings.websocketUrl === 'string' && settings.websocketUrl.trim()
...
export class WeComPerceptionPlugin implements PerceptionPlugin {
  ...
  constructor(private readonly createClient: WeComBotClientFactory = defaultClientFactory) {}
  ...
  async start(context: PerceptionPluginRuntimeContext): Promise<void> {
    ...
    try {
      ...
      const secretRef = typeof context.settings.secretRef === 'string' ? context.settings.secretRef : '';
      if (!secretRef || !context.ports.credentials) throw new Error('WECOM_SECRET_MISSING');
      const secret = await context.ports.credentials.resolve(context.connectorId, secretRef);
      ...
      await context.ports.health?.report({ status: 'degraded', safeCode: configurationSafeCode(error) });
      throw error;
    }
  }
  ...
  private bind(context: PerceptionPluginRuntimeContext, client: WeComBotClient): void {
    client.on('authenticated', () => {
      void context.ports.health?.report({ status: 'healthy', detail: {
        connectionState: 'connected', reconnectCount: this.reconnectCounts.get(context.connectorId) ?? 0,
        lastSuccessAt: new Date().toISOString(),
      } });
    });
    client.on('reconnecting', () => {
      const reconnectCount = (this.reconnectCounts.get(context.connectorId) ?? 0) + 1;
      this.reconnectCounts.set(context.connectorId, reconnectCount);
      void context.ports.health?.report({ status: 'degraded', detail: { connectionState: 'reconnecting', reconnectCount } });
    });
    client.on('disconnected', () => {
      void context.ports.health?.report({ status: 'degraded', safeCode: 'WECOM_DISCONNECTED', detail: {
        connectionState: 'disconnected', reconnectCount: this.reconnectCounts.get(context.connectorId) ?? 0,
      } });
    });
    client.on('error', (error) => {
      void context.ports.health?.report({ status: 'degraded', safeCode: connectionSafeCode(error), detail: {
        connectionState: 'disconnected', reconnectCount: this.reconnectCounts.get(context.connectorId) ?? 0,
      } });
    });
  ...
  private async submit(context: PerceptionPluginRuntimeContext, client: WeComBotClient, frame: WeComFrame): Promise<void> {
    ...
    try {
      ...
      const responses = dispatched?.responseTexts?.length
        ...
        : dispatched?.responseText ? [dispatched.responseText] : [];
      const streamId = `perception-${event.id}`;
      for (let index = 0; index < responses.length; index += 1) {
        await client.replyStream(frame, streamId, limitReply(responses[index] ?? ''), index === responses.length - 1);
      }
    } catch {
...
function connectionSafeCode(error: unknown): string {
  ...
  if (message.includes('authentication failed') || message.includes('auth failure')) return 'WECOM_AUTH_FAILED';
  if (message.includes('enotfound') || message.includes('getaddrinfo')) return 'WECOM_DNS_FAILED';
  if (message.includes('etimedout') || message.includes('timeout')) return 'WECOM_NETWORK_TIMEOUT';
  if (message.includes('certificate') || message.includes('tls') || message.includes('ssl')) return 'WECOM_TLS_FAILED';
  if (message.includes('econn') || message.includes('network')) return 'WECOM_NETWORK_UNAVAILABLE';
